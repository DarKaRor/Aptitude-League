...
public class MazeGenerator : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.Z))  GenerateMaze(10, 10);
    ...
    private void GenerateMaze(int rows, int columns)
    {
        ...
        mazeColumns = columns;
        CreateLayout();
        DeleteRepeatedWalls(); // Edit
        ResizeMaze(); // Edit
        DeleteLogicInCenter(); // Edit;
    ...
    public void ResizeMaze(float size = 2)
    {
        mazeParent.transform.localScale = new Vector2(size, size);
    }

    public void DeleteLogicInCenter()
    {
        foreach (Cell cell in centreCells) Destroy(cell.cellObject.GetComponent<TrashTile>());
    }

    public void DeleteRepeatedWalls()
    {
        foreach (KeyValuePair<Vector2, Cell> cell in allCells)
        {
            Vector2 cPos = cell.Key;
            Cell cCell = cell.Value;
            ...
            foreach (Vector2 p in neighbourPositions)
            {
                ...
                Vector2 nPos = cPos + p;
                // If cell exists.
                if (allCells.ContainsKey(nPos))
                {
                    nCell = allCells[nPos];

                    if (cCell.gridPos.x < nCell.gridPos.x)
                    {
                        RemoveWall(cCell.cScript, 2);
                    }

                    if(cCell.gridPos.y < nCell.gridPos.y)
                    {
    ...
    public void GenerateCell(Vector2 pos, Vector2 keyPos)
    {
        ...
        newCell.cellObject.transform.localScale = new Vector2(0.391f, 0.391f); // Edit
